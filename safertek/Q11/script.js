...
// Function to display employee details when clicking on the ID or View button
function displayEmployeeDetails(employeeId) {
    const employee = employees.find(emp => emp.id === employeeId);
    if (employee) {
        // Here, you can implement how you want to display the details,
        // such as in a modal, a separate page, or in a side panel.
...
// Add event listener to rows to display employee details on click
document.getElementById("employeeTable").addEventListener("click", function(event) {
    if (event.target.tagName === "TD" && event.target.parentNode.cells[0] === event.target) {
        const employeeId = parseInt(event.target.textContent);
